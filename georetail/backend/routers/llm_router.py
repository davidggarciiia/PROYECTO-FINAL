...
# Cada función lee la API key desde la variable de entorno correspondiente.
# Si la variable está vacía el cliente se crea de todas formas; fallará en la
# primera llamada con un error de autenticación que se captura y salta al
...
def _deep():
    ...
    global _DEEP
    if not _DEEP: _DEEP = AsyncOpenAI(
...
def _kimi():
    ...
    global _KIMI
    if not _KIMI: _KIMI = AsyncOpenAI(
...
async def completar(mensajes: list[dict], sistema: str, endpoint: str,
    ...
    for proveedor in ["anthropic", "openai", "deepseek", "kimi", "gemini"]:
        if await r.get(f"llm:exhausted:{proveedor}"):
            continue  # proveedor marcado como agotado temporalmente → saltar
