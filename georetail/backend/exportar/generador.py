...
async def encolar_generacion_pdf(
    ...
    """
    Encola la generación del PDF. Intenta usar Celery; si no está disponible,
    ejecuta la generación directamente de forma asíncrona.
    """
    try:
        from workers.tasks import generar_pdf_task
        generar_pdf_task.delay(pdf_id, session_id, zona_ids, opciones)
    except Exception:
        # Fallback: generación directa si Celery no está corriendo
        await generar(pdf_id=pdf_id, session_id=session_id, zona_ids=zona_ids, opciones=opciones)
