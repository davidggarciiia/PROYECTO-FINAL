...
export default function SearchBox({ onResults, sessionId }: Props) {
  ...
  const buscar = useCallback(async (descripcion: string, sid?: string) => {
    ...
    try {
      ...
      setError("Error al conectar con el servidor. ¿Está el backend en marcha?");
    } finally {
      setLoading(false);
    }
  ...
  const responderCuestionario = useCallback(async (respuesta: string) => {
    ...
    try {
      ...
      if (res.trigger_busqueda) {
        setPregunta(null);
        // Re-lanzar búsqueda con la sesión actualizada
    ...
    <div className={styles.container}>
      ...
      {pregunta && (
        <div className={styles.pregunta}>
          <span className={styles.preguntaLabel}>Cuestionario</span>
          <p>{pregunta}</p>
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className={styles.form}>
        <textarea
          ...
          placeholder={pregunta ? "Escribe tu respuesta..." : "Ej: quiero abrir un restaurante de tapas con terraza..."}
          rows={2}
          disabled={loading}
        ...
        >
          {loading ? <span className="spinner" /> : pregunta ? "Responder" : "Buscar"}
        </button>
      </form>
